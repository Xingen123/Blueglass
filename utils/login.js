...
const baseUrl = "https://wxmp.clicksdiy.com/";//测试环境
// const dataUrl = "https://wp.xxxx.com/wechat/";//数据统计测试环境
...
function wxLogin(loading, url, sessionChoose, sessionId, params, method, ask, callBack) {
  wx.login({
    success: function (res) {
      ...
      HttpRequst(true, "mini/program/onLogin", false, "", { "code": code }, "GET", false, function (res) {
        if (res.code == 200) {
          wx.setStorageSync('sessionId', res.sessionId);
          if (res.isNeedUserInfo == true) {
            wx.getUserInfo({
              success: function (res) {
                HttpRequst(true, "mini/program/onLogin", 3, wx.getStorageSync("sessionId"), { "encryptedData": res.encryptedData, "iv": res.iv }, "POST", false, function (res) {
